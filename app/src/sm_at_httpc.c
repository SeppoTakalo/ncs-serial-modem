...
enum sm_http_content_type {
	HTTP_CONTENT_TYPE_JSON = 0,   /* application/json */
	HTTP_CONTENT_TYPE_FORM = 1,   /* application/x-www-form-urlencoded */
	HTTP_CONTENT_TYPE_BINARY = 2, /* application/octet-stream */
	HTTP_CONTENT_TYPE_XML = 3,    /* application/xml */
	HTTP_CONTENT_TYPE_TEXT = 4,   /* text/plain */
};
...
struct http_request {
	...
	enum sm_http_content_type content_type;       /* Content-Type for POST/PUT */
	uint8_t recv_buf[HTTP_RECV_BUF_SIZE];         /* Receive buffer */
	int recv_buf_len;                             /* Bytes in receive buffer */
	int recv_buf_offset;                          /* Offset in receive buffer */
...
static int http_build_request(struct http_request *req, char *buf, size_t buf_len)
{
	...
	case HTTP_CONTENT_TYPE_BINARY:
		...
		break;
	case HTTP_CONTENT_TYPE_XML:
		content_type_str = "application/xml";
		break;
	case HTTP_CONTENT_TYPE_TEXT:
		content_type_str = "text/plain";
		break;
	default:
		content_type_str = "application/json"; /* Default to JSON */
		break;
	}
	...
	if (req->request_body_len > 0) {
		len += snprintf(buf + len, buf_len - len,
				"Content-Length: %d\r\n"
				"Content-Type: %s\r\n",
				req->request_body_len, content_type_str);
	}
			      ...
			      uint32_t param_count)
{
	...
	case AT_PARSER_CMD_TYPE_SET: {
		...
		int body_len = 0;
		int content_type = HTTP_CONTENT_TYPE_JSON; /* Default to JSON */
		...
		/* Get optional content type for POST/PUT */
		if (param_count > next_param_idx && (method == HTTP_POST || method == HTTP_PUT)) {
			err = at_parser_num_get(parser, next_param_idx, &content_type);
			if (err && err != -ENOENT) {
				LOG_ERR("Failed to parse content_type: %d", err);
				return err;
			}
			if (err != -ENOENT) {
				if (content_type < HTTP_CONTENT_TYPE_JSON ||
				    content_type > HTTP_CONTENT_TYPE_TEXT) {
					LOG_ERR("Invalid content_type: %d", content_type);
					return -EINVAL;
				}
				LOG_DBG("Content type: %d", content_type);
			}
		}
