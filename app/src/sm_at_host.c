...
int sm_at_send_str(const char *str)
{
	...
	return sm_at_send_internal(ctx, (const uint8_t *)str, strlen(str), false,
				   SM_DEBUG_PRINT_FULL);
}
...
static void cmd_send(uint8_t *buf, size_t cmd_length)
{
	int err;
	size_t offset = 0;
	char *at_cmd = buf;
	...
	if (strlen(sm_response_buf) > strlen(CRLF_STR)) {
		...
		err = sm_at_send_str(sm_response_buf);
		if (err) {
...
static size_t cmd_rx_handler(struct sm_at_host_ctx *ctx, uint8_t c)
{
	...
	if (send) {
		if (ctx->at_cmd_len > ctx->at_buf_size - 1) {
			...
			cmd_send(ctx->at_buf, ctx->at_cmd_len);
		} else {
...
static void sm_at_host_work_fn(struct k_work *work)
{
	...
	while (k_msgq_get(&sm_at_host_msgq, &msg, K_NO_WAIT) == 0) {
		LOG_DBG("AT host msg received: pipe_event=%s, sm_event=%s (ctx=%p, pipe=%p)",
			sm_pipe_event_str[msg.pipe_event], sm_event_str[msg.sm_event],
			(void *)msg.ctx, (void *)msg.pipe);
